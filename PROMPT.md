...
## 目标
根据所有的NPC数据（50+） 和兽王数据（10+），反向推导并补全地理与门派信息，构建一个逻辑自洽的宏大江湖。
...
## 验收标准
- `spatial_nodes.md` 覆盖所有 NPC (50+) 所在的区域，无“悬空”角色。
- `spatial_nodes.md` 完整整合所有兽王 (17+) 的栖息地分布信息。
